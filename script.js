     ...
     function waitForGoogleScript(callback, timeout = 10000, interval = 500) {
         ...
         function check() {
             if (typeof google !== 'undefined' && google.script && google.script.run) {
                 ...
                 console.error('Erro: Biblioteca do Google Apps Script não carregada após o tempo limite.');
                 const errorMessage = document.getElementById('identificacao-error');
                 errorMessage.style.display = 'block';
                 errorMessage.textContent = 'Erro: Não foi possível conectar ao servidor. Tente recarregar a página.';
             }
         }
         check();
     }
